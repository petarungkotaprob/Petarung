...
function inverse3(p) {
  ...
  if (!this.es) {
    ...
    if (Math.abs(phi) < HALF_PI) {
      ...
      lon = adjust_lon_default(this.long0 + d2 * (1 - ds / 6 * (1 + 2 * t + c - ds / 20 * (5 + 28 * t + 24 * ts + 8 * c * t + 6 * c - ds / 42 * (61 + 662 * t + 1320 * ts + 720 * ts * t)))) / cos_phi);
    } else {
...
function forward9(p) {
  ...
  if (Math.abs(Math.abs(p.y) - HALF_PI) > EPSLN) {
    ...
    U2 = (S * this.singam - V2 * this.cosgam) / T;
    ...
    if (Math.abs(temp) < TOL) {
      ...
      u2 = this.ArB * Math.atan2(S * this.cosgam + V2 * this.singam, temp);
    }
  } else {
...
function inverse11(p) {
  ...
  var iter = 0;
  do {
    p.y = 2 * (Math.atan(Math.pow(this.k, -1 / this.alfa) * Math.pow(Math.tan(u2 / 2 + this.s45), 1 / this.alfa) * Math.pow((1 + this.e * Math.sin(fi1)) / (1 - this.e * Math.sin(fi1)), this.e / 2)) - this.s45);
...
function forward24(p) {
  ...
  x = this.x0 + con;
  var q = asq + g2;
  con = Math.PI * this.R * (m2 * q - al * Math.sqrt((msq + asq) * (asq + 1) - q * q)) / (msq + asq);
  if (lat >= 0) {
    y = this.y0 + con;
  } else {
    y = this.y0 - con;
...
var d;
var g = () => new DecompressionStream("deflate-raw");
try {
  g(), d = async (t) => {
...
var regex = /.+\.(shp|dbf|json|prj|cpg)$/i;
  ...
  if (orphens.length && !reversed) {
    ...
    if (otherPosibility.orphens.length === 0) {
      ...
      for (const outer of otherPosibility.outers) {
        out2.push([outer.ring.toReversed()].concat(outer.children.map((item) => item.toReversed())));
      }
      return out2;
    }
  }
...
function createDecoder(encoding, second) {
  ...
  try {
    ...
    var match2 = regex2.exec(encoding);
    if (match2 && !second) {
...
var parseZip = async function(buffer, whiteList) {
  ...
  for (key in zip) {
    ...
    if (key.slice(-4).toLowerCase() === ".shp") {
      names36.push(key.slice(0, -4));
      zip[key.slice(0, -3) + key.slice(-3).toLowerCase()] = zip[key];
    } else if (key.slice(-4).toLowerCase() === ".prj") {
      zip[key.slice(0, -3) + key.slice(-3).toLowerCase()] = lib_default(zip[key]);
    } else if (key.slice(-5).toLowerCase() === ".json" || whiteList.indexOf(key.split(".").pop()) > -1) {
      names36.push(key.slice(0, -3) + key.slice(-3).toLowerCase());
  ...
  const geojson = names36.map(function(name) {
    let parsed, dbf;
    ...
    if (lastDotIdx > -1 && name.slice(lastDotIdx).indexOf("json") > -1) {
      ...
      if (zip[name + ".dbf"]) {
        dbf = parsedbf_default(zip[name + ".dbf"], zip[name + ".cpg"]);
      }
...
var checkSuffix = (base, suffix) => {
  var _a;
  const url = new URL2(base, (_a = globalThis == null ? void 0 : globalThis.document) == null ? void 0 : _a.location);
